...
object Immutable {
  def safe[@spec A:ArrayTag](as:Array[A]) = new Forward(as.clone, 0, as.length)
  def unsafe[@spec A:ArrayTag](as:Array[A]) = new Forward(as, 0, as.length)
...
trait Immutable[@spec A] extends Buffer[A] {
  implicit def manifest:ArrayTag[A]
...
final class Forward[@spec A](elems:Array[A], start:Int, end:Int)(implicit val manifest:ArrayTag[A]) extends Immutable[A] {
  ...
  def map[@spec B:ArrayTag](f:A => B) = Immutable.unsafe(mapBufferToArray(this, start, length)(f))
}

final class Reversed[@spec A](elems:Array[A], start:Int, end:Int)(implicit val manifest:ArrayTag[A]) extends Immutable[A] {
...
final class Lazy[@spec A, @spec U](us:Immutable[U])(f:U => A)(implicit val manifest:ArrayTag[A]) extends Immutable[A] {
  ...
  def map[@spec B:ArrayTag](f:A => B) = new Lazy(us)(u => f(this.f(u)))
}
