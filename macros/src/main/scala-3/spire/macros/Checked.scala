...
object Checked:
  ...
  private def toInt[A](n: Expr[A])(using Quotes, Type[A]): Expr[Int] =
    ...
    if (n.isExprOf[Int]) n.asExprOf[Int]
    else if (n.isExprOf[Byte]) '{ ${ n.asExprOf[Byte] }.toInt }
    else if (n.isExprOf[Short]) '{ ${ n.asExprOf[Short] }.toInt }
    else
  ...
  private def toLong[A](n: Expr[A])(using Quotes, Type[A]): Expr[Long] =
    import quotes.reflect.*
    if (n.isExprOf[Int]) '{ ${ n.asExprOf[Int] }.toLong }
    else if (n.isExprOf[Byte]) '{ ${ n.asExprOf[Byte] }.toLong }
    else if (n.isExprOf[Short]) '{ ${ n.asExprOf[Short] }.toLong }
    else if (n.isExprOf[Long]) n.asExprOf[Long]
    else
  ...
  inline def option[A](inline n: A): Option[A] =
    try Some(checked(n))
    catch case a: ArithmeticOverflowException => None
  ...
  inline def tryOrElse[A](inline n: A)(orElse: => A): A =
    try checked(n)
    catch case a: ArithmeticOverflowException => orElse
