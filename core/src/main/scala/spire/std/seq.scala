 ...
 */
@SerialVersionUID(0L)
class SeqLpNormedVectorSpace[A: Field: NRoot: Signed, SA <: SeqOps[A, Seq, SA]](val p: Int)(implicit
  cbf: Factory[A, SA]
) extends SeqVectorSpace[A, SA]
...
private object SeqSupport {
  ...
  @inline final def forall[A, SA <: SeqOps[A, Seq, SA]](x: SA,
                                                        y: SA
...
trait SeqInstances2 extends SeqInstances1 {
  implicit def SeqInnerProductSpace[A, CC[A] <: SeqOps[A, Seq, CC[A]]](implicit
    field0: Field[A],
    cbf0: Factory[A, CC[A]]
  ): SeqInnerProductSpace[A, CC[A]] = new SeqInnerProductSpace[A, CC[A]]
...
trait SeqInstances3 extends SeqInstances2 {
  implicit def SeqNormedVectorSpace[A, CC[A] <: SeqOps[A, Seq, CC[A]]](implicit
    field0: Field[A],
    nroot0: NRoot[A],
    cbf0: Factory[A, CC[A]]
  ): NormedVectorSpace[CC[A], A] = SeqInnerProductSpace[A, CC].normed
}
