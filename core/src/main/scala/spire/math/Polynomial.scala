...
object Polynomial {
  ...
  def one[@spec(Double) C: Eq: Ring: ClassTag] =
    constant(Ring[C].fromInt(1))
  def x[@spec(Double) C: Eq: Ring: ClassTag] =
    linear(Ring[C].fromInt(1))
  def twox[@spec(Double) C: Eq: Ring: ClassTag] =
    linear(Ring[C].fromInt(2))
