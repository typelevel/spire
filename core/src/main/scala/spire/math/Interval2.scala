...
sealed abstract class Interval2[A](implicit order: Order[A]) { lhs =>
  ...
  def isSupersetOf(rhs: Interval2[A]): Boolean = lhs match {
    case All() =>
      true
  ...
  def isSubsetOf(rhs: Interval2[A]): Boolean =
  ...
  def mask(rhs: Interval2[A]): Interval2[A] = lhs match {
    ...
    case Below(upper1, flags1) => rhs match {
      ...
      case Above(lower2, flags2) =>
        Interval2.withFlags(lower2, upper1, flags1 | flags2)
      case Ranged(lower2, upper2, flags2) =>
        val (u, uf) = minUpper(upper1, upper2, flags1, flags2)
        Interval2.withFlags(lower2, u, lowerFlag(flags2) | uf)
    }
    case Above(lower1, flags1) => rhs match {
      ...
      case Above(lower2, flags2) =>
        ...
        Above(l, lf)
      case Below(upper2, flags2) =>
        Interval2.withFlags(lower1, upper2, flags1 | flags2)
      case Ranged(lower2, upper2, flags2) =>
        val (l, lf) = maxLower(lower1, lower2, flags1, flags2)
        Interval2.withFlags(l, upper2, lf | upperFlag(flags2))
    }
    case Ranged(lower1, upper1, flags1) =>
      rhs match {
        case All() =>
        ...
        case Above(lower2, flags2) =>
          val (l, lf) = maxLower(lower1, lower2, flags1, flags2)
          Interval2.withFlags(l, upper1, lf | upperFlag(flags1))
        case Below(upper2, flags2) =>
          val (u, uf) = minUpper(upper1, upper2, flags1, flags2)
          Interval2.withFlags(lower1, u, lowerFlag(flags1) | uf)
        case Ranged(lower2, upper2, flags2) =>
          val (l, lf) = maxLower(lower1, lower2, flags1, flags2)
          val (u, uf) = minUpper(upper1, upper2, flags1, flags2)
          Interval2.withFlags(l, u, lf | uf)
      }
  }
  ...
  def combine(rhs: Interval2[A])(f: (A, A) => A): Interval2[A] = {
  ...
  def +(rhs: Interval2[A])(implicit ev: AdditiveSemigroup[A]): Interval2[A] =
    combine(rhs)(_ + _)

  def -(rhs: Interval2[A])(implicit ev: AdditiveGroup[A]): Interval2[A] =
    combine(rhs)(_ - _)
  ...
  private[this] def fromTpl(t1: (A, Int), t2: (A, Int)): Interval2[A] =
    Interval2.withFlags(t1._1, t2._1, lowerFlag(t1._2) | lowerFlagToUpper(t2._2))

  def *(rhs: Interval2[A])(implicit ev: Ring[A]): Interval2[A] = {
    if (lhs.isEmpty || rhs.isEmpty) return Interval2.empty[A]
    val z = ev.zero
    if (lhs.isAt(z) || rhs.isAt(z)) return Interval2.point(z)
  ...
  def /(rhs: Interval2[A])(implicit ev: Field[A]): Interval2[A] = {
    val z = ev.zero
          ...
          lhs match {
            ...
            case Ranged(lower1, upper1, flags1) =>
              if (lower1 >= z) {
                ...
                Ranged(upper1 / lower2, lower1 / lower2, swapFlags(flags1) | lowerFlag(flags2) | lowerFlagToUpper(flags2))
              } else {
  ...
  def +(rhs: A)(implicit ev: AdditiveSemigroup[A]): Interval2[A] =
    this match {
  ...
  def -(rhs: A)(implicit ev: AdditiveGroup[A]): Interval2[A] =
    this + (-rhs)

  def unary_-()(implicit ev: AdditiveGroup[A]): Interval2[A] =
    this match {
  ...
  def *(rhs: A)(implicit ev: Ring[A]): Interval2[A] =
    if (rhs < ev.zero) {
      this match {
...
case class All[A: Order] private[spire] () extends Interval2[A]
case class Above[A: Order] private[spire] (lower: A, flags: Int) extends Interval2[A]
case class Below[A: Order] private[spire] (upper: A, flags: Int) extends Interval2[A]
case class Ranged[A: Order] private[spire] (lower: A, upper: A, flags: Int) extends Interval2[A]

object Interval2 {
  private[spire] def withFlags[A: Order](lower: A, upper: A, flags: Int): Interval2[A] =
  ...
  def point[A: Order](a: A): Interval2[A] =
    Ranged(a, a, 0)

  def zero[A](implicit o: Order[A], r: Ring[A]): Interval2[A] =
    Ranged(r.zero, r.zero, 0)

  def all[A: Order]: Interval2[A] = All[A]()

  def closed[A: Order](lower: A, upper: A): Interval2[A] =
    if (lower <= upper) Ranged(lower, upper, 0) else Interval2.empty[A]
  def open[A: Order](lower: A, upper: A): Interval2[A] =
    if (lower <= upper) Ranged(lower, upper, 3) else Interval2.empty[A]
  def openBelow[A: Order](lower: A, upper: A): Interval2[A] =
    if (lower < upper) Ranged(lower, upper, 1) else Interval2.empty[A]
  def openAbove[A: Order](lower: A, upper: A): Interval2[A] =
    if (lower < upper) Ranged(lower, upper, 2) else Interval2.empty[A]
