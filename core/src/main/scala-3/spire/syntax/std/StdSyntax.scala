...
trait IntSyntax:
  extension (lhs: Int) def /~(rhs: Int): Int = lhs / rhs
...
trait LongSyntax:
  extension (lhs: Long) def /~(rhs: Long): Long = lhs / rhs
...
trait DoubleSyntax:
  extension (lhs: Double) def pow(rhs: Double): Double = spire.math.pow(lhs, rhs)
  def **(rhs: Double): Double = spire.math.pow(lhs, rhs)
end DoubleSyntax

trait BigIntSyntax:
  extension (lhs: BigInt) def /~(rhs: BigInt): BigInt = lhs / rhs
