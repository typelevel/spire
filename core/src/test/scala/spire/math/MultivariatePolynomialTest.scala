...
class MultivariatePolynomialCheck extends PropSpec with ShouldMatchers with GeneratorDrivenPropertyChecks {
  ...
  def runLex[A: Arbitrary: Eq: Field: ClassTag](typ: String) {
    implicit val arb: Arbitrary[MultivariatePolynomial[A]] = Arbitrary(for {
      ...
      if(!ts.isEmpty && ts.length <= 5)
    } yield {
      MultivariatePolynomial[A](ts)
    })
    runTest[A](s"$typ/lexicographic multivariate polynomial")
  }
  ...
  def runMonomialTest[A: Eq: ClassTag](name: String)(implicit arb: Arbitrary[Monomial[A]], f: Ring[A]) {
    type M = Monomial[A]
    ...
    property(s"$name m = m") {
      forAll { (m: M) => println(s"monomial test case = $m"); m should be === m }
    }
  ...
  def runTest[A: Eq: ClassTag](name: String)(implicit arb: Arbitrary[MultivariatePolynomial[A]], f: Field[A]) {
    ...
    property(s"$name p = p") {
      forAll { (p: P) => println(s"MVP test case = $p"); p should be === p }
    }
