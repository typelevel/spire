...
object OldRingLaws {
  def apply[A : Eq : Arbitrary](implicit _pred: Predicate[A]) = new OldRingLaws[A] {
...
trait OldRingLaws[A] extends OldGroupLaws[A] {

  // must be a val (stable identifier)
  val nonZeroLaws: OldGroupLaws[A]
  def pred: Predicate[A]

  def withPred(_pred: Predicate[A], replace: Boolean = true): OldRingLaws[A] = OldRingLaws[A](
    Equ,
    Arb,
  ...
  class MultiplicativeProperties(
    ...
    private val _base = base(OldRingLaws.this)
