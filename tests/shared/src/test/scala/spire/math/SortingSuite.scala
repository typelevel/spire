...
class SortingSuite extends munit.FunSuite {
  ...
  test("MergeSort.merge: end cannot exceed the size of the output array") {
    intercept[IllegalArgumentException] { MergeSort.merge(Array(5, 3, 4, 7, 5, 8, 9),
  ...
  test("QuickSort.partition: 0 <= start <= pivot < end <= length of input") {
    ...
    Array((-1, 2, 1), (0, 5, 2), (0, 4, 4), (2, 4, 1), (1, 3, 4)).foreach {
      case (start, end, pivotIndex) => intercept [IllegalArgumentException] { QuickSort.partition(input, start, end, pivotIndex) }
    }
  }
  ...
  private def checkMutation[A: Order : ClassTag](mutation: Array[A] => Unit, toMutate: Array[A])(check: Array[A] => Unit) = {
