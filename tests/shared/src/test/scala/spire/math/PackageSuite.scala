...
class PackageSuite extends munit.FunSuite {
  ...
  test("pow(BigDecimal, BigDecimal)") {
    ...
    assertEquals(pow(BigDecimal("1.00000001"), BigDecimal("2000000000")), BigDecimal("485165146.8932734862594042197965646"))

    // wolfram alpha says:                                                               1.007169457206495657176120750097341
    assertEquals(pow(BigDecimal(Double.MaxValue) * 100, BigDecimal("0.00001")), BigDecimal("1.007169457206495657262139152506902"))
  }
