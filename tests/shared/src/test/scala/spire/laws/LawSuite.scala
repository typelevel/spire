...
class LawSuite extends munit.DisciplineSuite {
  ...
  checkAll("List[Rational]", VectorSpaceLaws[List[Rational], Rational].normedVectorSpace(max, implicitly, implicitly))
  ...
  checkAll("LatticePartialOrder[Int]",
           LatticePartialOrderLaws[Int].boundedLatticePartialOrder(intMinMaxLattice, implicitly[Order[Int]])
  )
