...
class PartialOrderSuite extends munit.FunSuite {
  ...
  implicit val po: PartialOrder[Set[Int]] = PartialOrder[Set[Int]]
  test("Minimal and maximal elements of {{1, 2, 3}, {3}, {2}, {1}} by power set partial order") {
    ...
    assertEquals(seqOps(sets).pmin.toSet, Set(Set(1), Set(2), Set(3)))
    assertEquals(seqOps(sets).pmax.toSet, Set(Set(1, 2, 3), Set(1, 4)))
  }
