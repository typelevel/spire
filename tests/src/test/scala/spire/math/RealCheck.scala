...
class RealCheck extends PropSpec with Matchers with GeneratorDrivenPropertyChecks {
  ...
  def runSample(f: Real => Real, x0: Rational, i0: Byte, j0: Byte): Unit = {
    val x = f(Real(x0.abs))
    val i = (i0 & 0x3f) + 1
    val j = (j0 & 0x3f) + 1
    ...
    val v1 = x(k1)
    val v2 = x(k2)
    val d = SafeLong(2).pow(k2 - k1)
    val r = Rational(v2, d)
    val v3 = Real.floor(r)
    
    val s1 = Rational(2).pow(k1) * x
    val s2 = Rational(2).pow(k2) * x
    
    if (v1 != v3) {
      println(s"x=$x (${x.toBigDecimal})  k1=$k1  k2=$k2  v1=$v1  v2=$v2  d=$d  r=$r  v3=$v3  s1=${s1.toBigDecimal}  s2=${s2.toBigDecimal}")
    }
    v1 shouldBe v3
    //Real.floor(v2 >> (k2 - k1)) shouldBe v1
  }

  def sample1(name: String)(f: Real => Real): Unit =
    property(name) {
      forAll { (x0: Rational, i0: Byte, j0: Byte) =>
        runSample(f, x0, i0, j0)
      }
    }

  property("a") { runSample(x => x, Rational("990475272870381/4835703278458516698824704"), 101, 102) }
  property("b") { runSample(x => -x, Rational("990475272870381/4835703278458516698824704"), 101, 102) }
