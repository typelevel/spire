...
class FixedPoint(val long: Long) extends AnyVal { lhs =>
  ...
  def t_%(rhs: FixedPoint): FixedPoint =
    new FixedPoint(lhs.long % rhs.long)

  def t_%(rhs: Long)(implicit scale: FixedScale): FixedPoint = {
    val d = scale.denom
    val p = rhs * d
  ...
  def t_/~(rhs: FixedPoint)(implicit scale: FixedScale): FixedPoint = (lhs - lhs t_% rhs) / rhs

  def t_/%(rhs: FixedPoint)(implicit scale: FixedScale): (FixedPoint, FixedPoint) = {
    val rem = lhs t_% rhs
    ((lhs - rem) / rhs, rem)
  }
  ...
  // TODO: check for correctness this GCD/LCM
...
trait FixedPointInstances {
  ...
  implicit def algebra(implicit scale: FixedScale): Fractional[FixedPoint] with Order[FixedPoint] with Signed[FixedPoint] =
    new Fractional[FixedPoint] with Order[FixedPoint] with TruncatedDivisionCRing[FixedPoint] with Field.WithDefaultGCD[FixedPoint] { self =>
      ...
      def tquot(x: FixedPoint, y: FixedPoint): FixedPoint = x t_/~ y
      def tmod(x: FixedPoint, y: FixedPoint): FixedPoint = x t_% y
